...
const STATUS_MESSAGES_FILE = process.env.STATUS_MESSAGES_FILE || path.join(__dirname, '..', 'config', 'statusMessages.json');
const SERVER_METADATA_FILE = process.env.SERVER_METADATA_FILE || path.join(__dirname, '..', 'config', 'serverMetadata.json');
...
function loadStatusMessages() {
  if (fs.existsSync(STATUS_MESSAGES_FILE)) {
    try { return fs.readJsonSync(STATUS_MESSAGES_FILE); } catch (e) { return []; }
  }
  return [];
}
...
function loadServerMetadata() {
  if (fs.existsSync(SERVER_METADATA_FILE)) {
    try { return fs.readJsonSync(SERVER_METADATA_FILE); } catch (e) { return {}; }
  }
  return {};
}
...
function createStatusEmbed(server, status, metadata) {
  ...
  const description = metadata?.description || `Status monitor for ${server.name} on ${server.panel}`;
  const iconUrl = metadata?.iconUrl || null;
  
  let color = 0x808080; // Gray
  if (status === 'running') color = 0x00FF00; // Green
  else if (status === 'offline') color = 0xFF0000; // Red
...
async function updateAllEmbeds(client, getAllServers) {
  ...
  for (const trackInfo of messages) {
    ...
    try {
      ...
      console.error(`Failed to update status message ${messageId}: ${err.message}. Removing from tracking.`);
  ...
  if (invalidMessageIds.length > 0) {
    ...
    currentMessages = currentMessages.filter(m => !invalidMessageIds.includes(m.messageId));
